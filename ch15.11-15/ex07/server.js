...
function cspMiddleware(_url, req, res) {
  // ricohHash で、インラインスクリプトのうち、RICOH を許可
  const ricohHash = crypto
    .createHash('sha256')
    .update('alert("RICOH")')
    .digest('base64');

  // helloSrc で外部スクリプト hello.js のパスを許可
  const helloSrc = `http://${req.headers.host}/hello.js`;

  // ※hash はインラインスクリプト専用。外部スクリプトに対して、hash は無効
  res.setHeader(
    'Content-Security-Policy',
    `script-src 'sha256-${ricohHash}' ${helloSrc}`
  );
