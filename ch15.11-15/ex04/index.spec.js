...
test.describe('localStorage版ToDoアプリ', () => {
  ...
  test('複数タブ間でToDoが同期される', async ({ context }) => {
    // 1つ目のページを開く
    ...
    // page1でToDoを追加
    await page1.getByRole('textbox').fill('Synced Task');
    await page1.getByRole('button', { name: 'Add' }).click();

    // 少し待機してstorageイベントが発火するのを待つ
    await page2.waitForTimeout(500);

    // page2で追加されたToDoが表示されることを確認
  ...
  test('一方のタブで削除すると他方のタブにも反映される', async ({
    ...
    // ToDoを追加
    await page1.getByRole('textbox').fill('Task to sync delete');
    await page1.getByRole('button', { name: 'Add' }).click();
  ...
  test('localStorageが使用できない場合でもエラーが発生しない', async ({
    page,
  }) => {
    // localStorageを無効化
    ...
    // ただしリロード後は消える（localStorageが使えないため）
    await page.reload();
