...
// sessionStorage が利用可能かチェックする関数
function isSessionStorageAvailable() {
  try {
    const test = '__sessionStorage_availability_test__';
...
// sessionStorage に ToDo リストを保存する
function saveTodos() {
  if (!isSessionStorageAvailable()) return;

  try {
    sessionStorage.setItem(STORAGE_KEY, JSON.stringify({ todos, nextId }));
  } catch (e) {
...
// sessionStorageは同じタブ内でのみ有効なため、storageイベントは発生しない
// (localStorageと異なり、タブ間での同期は行われない)
