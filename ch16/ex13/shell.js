    ...
    case '>': // RedirCmd
      {
        ...
        const fileStream = fs.createWriteStream(cmd.file); // ファイルへの書き込みストリームを作成
        await runcmd(cmd.cmd, stdin, fileStream); // ストリームを runcmd に渡す
      }
      break;
    ...
    case '<': // RedirCmd
      {
        ...
        const fileStream = fs.createReadStream(cmd.file); // ファイルからの読み取りストリームを作成
        await runcmd(cmd.cmd, fileStream, stdout); // ストリームを runcmd に渡す
      }
      break;
    ...
    case '|': // PipeCmd
      {
        ...
        const pipe = new PassThrough(); // パイプ用のストリームを作成
        await Promise.all([
          runcmd(cmd.left, stdin, pipe), // 左コマンドの出力をパイプに接続
          runcmd(cmd.right, pipe, stdout), // 右コマンドの入力をパイプから接続
        ]);
      }
      break;
