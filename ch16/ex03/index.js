...
async function readKey() {
  ...
  return Buffer.from(
    JSON.parse(await fs.promises.readFile('key.json', 'utf8')).key,
    'base64'
  );
}
...
async function readEncrypt64() {
  ...
  return JSON.parse(await fs.promises.readFile('encryptedData.json', 'utf8'));
}
...
function decrypt64(data, key) {
  ...
  return decipher.update(data.value, 'base64', 'utf8') + decipher.final('utf8');
}
