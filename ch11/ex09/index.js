...
export function seq(...pats) {
  ...
  return pats.reduce(seq2, (str, pos, k) => k(str, pos));
}

// alt2 の可変長引数版
export function alt(...pats) {
  // HINT: alt(p1, p2, p3, p4) =  alt2(alt2(alt2(p1, p2), p3), p4)
  return pats.reduce((acc, p) => alt2(acc, p));
}
...
export function charFrom(s) {
  // HINT: quote の実装を参考にすると良い
  return (str, pos, k) =>
    pos < str.length && s.includes(str[pos]) ? k(str, pos + 1) : false;
}
...
export function repeat(pat, min = 0, max = Infinity) {
  ...
  // seq2, alt2 を上手く使うと良い
  const rec = (str, pos, k, count) => {
    if (count >= min) if (k(str, pos)) return true;
    if (count < max) return pat(str, pos, (s, p) => rec(s, p, k, count + 1));
    return count >= min && k(str, pos);
  };
  return (str, pos, k) => rec(str, pos, k, 0);
}
